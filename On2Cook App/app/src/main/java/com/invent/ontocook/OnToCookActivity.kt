...
import com.invent.ontocook.view.ItemExpandableStepView
import com.masoudss.lib.Utils
import com.masoudss.lib.WaveGravity
import com.mohammedalaa.seekbar.Direction
import kotlinx.android.synthetic.main.activity_on_to_cook.*
import kotlinx.android.synthetic.main.activity_on_to_cook.btnNext
import kotlinx.android.synthetic.main.activity_on_to_cook.btnSkip
import kotlinx.android.synthetic.main.activity_on_to_cook.clCurrentRecipe
import kotlinx.android.synthetic.main.activity_on_to_cook.clNextStep
import kotlinx.android.synthetic.main.activity_on_to_cook.clPrePrepare
import kotlinx.android.synthetic.main.activity_on_to_cook.clSeekBarView
import kotlinx.android.synthetic.main.activity_on_to_cook.ivCenterPlay
import kotlinx.android.synthetic.main.activity_on_to_cook.ivCurrentProgress
import kotlinx.android.synthetic.main.activity_on_to_cook.ivCurrentPulseBg
import kotlinx.android.synthetic.main.activity_on_to_cook.ivPlayPause
import kotlinx.android.synthetic.main.activity_on_to_cook.ivPlaySound
import kotlinx.android.synthetic.main.activity_on_to_cook.ivPulseBg
import kotlinx.android.synthetic.main.activity_on_to_cook.nestedScrollSeekBar
import kotlinx.android.synthetic.main.activity_on_to_cook.stepProgress
import kotlinx.android.synthetic.main.activity_on_to_cook.tvDesc
import kotlinx.android.synthetic.main.activity_on_to_cook.tvPrePrepareDesc
import kotlinx.android.synthetic.main.activity_on_to_cook.tvPrePrepareTitle
import kotlinx.android.synthetic.main.activity_on_to_cook.tvStep
import kotlinx.android.synthetic.main.activity_on_to_cook.tvStepDescDash
import kotlinx.android.synthetic.main.activity_on_to_cook.tvStepTitle
import kotlinx.android.synthetic.main.activity_on_to_cook.tvTitle
import kotlinx.android.synthetic.main.activity_on_to_cook.viewContainer
import kotlinx.android.synthetic.main.activity_on_to_cook.viewMicrowaveProgress
import kotlinx.android.synthetic.main.activity_on_to_cook.viewStoveProgress
...
class OnToCookActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_on_to_cook)
        init()
    }
    ...
    override fun onBackPressed() {
        if (stepProgress.currentProgress < 99f) {
            val alertDialog: android.app.AlertDialog? =
                android.app.AlertDialog.Builder(this@OnToCookActivity).create()
...
//        }
    ...
    private fun prepareScaleTimeDialog(isInduction : Boolean = true){
        ...
        dialogScaleStepTime?.context = this@OnToCookActivity
...
//                        )
    ...
    fun init() {
        ...
        ivPlaySound.setOnClickListener {
            OnToCookApplication.instance.speak(getPreparationMessage())
        }
        ...
        tvSos.setOnClickListener {
            ...
            dialogPauseRecipeView.context = this@OnToCookActivity
...
//                prepareDetailsView()
        ...
        viewStoveProgress.apply {
            progress = 0.0F
            waveWidth = Utils.dp(this@OnToCookActivity, 4)
            waveGap = Utils.dp(this@OnToCookActivity, 4)
            waveMinHeight = Utils.dp(this@OnToCookActivity, 5)
            waveCornerRadius = Utils.dp(this@OnToCookActivity, 20)
            waveGravity = WaveGravity.RIGHT
            waveBackgroundColor = ContextCompat.getColor(this@OnToCookActivity, R.color.dark_grey5)
            waveProgressColor = ContextCompat.getColor(this@OnToCookActivity, R.color.colorMWColor)
        ...
        viewMicrowaveProgress.apply {
            ...
            waveBackgroundColor = ContextCompat.getColor(this@OnToCookActivity, R.color.dark_grey5)
            waveProgressColor =
                ContextCompat.getColor(this@OnToCookActivity, R.color.colorFlamColor)
...
//                )
    ...
    private fun setLayoutParam(topMargin: Int, bottomMargin: Int? = null) {
        ...
        (clCurrentRecipe.layoutParams as ConstraintLayout.LayoutParams).topMargin = topMargin
    }
    ...
    private fun prepareDetailsView(isUpdateMargin: Boolean = false) {
        if (isUpdateMargin) {
            ...
            for ((index, progress) in stepProgressList.withIndex()) {
                ...
                val rootView = ItemExpandableStepView(this@OnToCookActivity)
    ...
    private fun manageManualScroll(stepIndex: Int) {
        ...
        val index = viewContainer.children().withIndex().indexOfFirst {
            totalTime >= recipeStepList[it.index].durationInSec
                    && totalTime <= (recipeStepList[it.index].durationInSec + recipeStepList[it.index].stepDuration)
        }
        if (index != -1) {
    ...
    private fun getDummyWaveSample(isMagnetron: Boolean = false): IntArray {
        ...
        for ((index, recipeObj) in recipeStepList.withIndex()) {
            //Divide by 3 because we have set height of bar as 3 second (1 bar  = 3 sec..)
    ...
    private fun prepareProgressbar(isResetProgress: Boolean = true) {
        ...
        //setStepTime()
        stepProgress.progressBarHeight =
            Constants.getDp(stepSums * 3, this@OnToCookActivity).toFloat()
...
//    }
    ...
    private fun getPreparationMessage() : String{
        var message = ""
        if(isInitial == 1 && currentPrepareStep == 0){
            isInitial = 0
            message += "Pre Preparation."
        }
        message += "Step ${currentPrepareStep + 1}. ${recipe!!.Ingredients[currentPrepareStep].title}"
        return message
    }
